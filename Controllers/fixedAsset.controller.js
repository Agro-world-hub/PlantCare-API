...
//             if (beginErr) {
//                 connection.release();
//                 return res.status(500).json({ message: 'Error starting transaction', error: beginErr });
//             }})
...
//                         const machToolsWarrantySql = `INSERT INTO machtoolsfixedassetwarranty (machToolsId, purchaseDate, expireDate, warrantystatus)
//                                                       VALUES (?, ?, ?, ?)`;
//                         connection.query(machToolsWarrantySql, [machToolsId, formattedPurchaseDate, formattedExpireDate, warranty], (warrantyErr) => {
//                             if (warrantyErr) {
//                                 return db.plantcare.rollback(() => {
//                                     return res.status(500).json({ message: 'Error inserting into machtoolsfixedassetwarranty table', error: warrantyErr });
//                                 });
...
// exports.updateFixedAsset = (req, res, connection) => {
//     const userId = req.user.id;
//     const { assetId, category } = req.params;
//     const assetData = req.body;
...
// // Helper functions for executing delete/insert and update sequences
// function executeDeleteAndInsertQueries(res, deleteQueries, deleteParams, insertQueries, insertParams, updateAssetQuery, updateParams, connection) {
...
//         })
//         .catch((err) => {
//             connection.rollback(() => res.status(500).json({ message: 'Error executing ownership change', error: err }));
//         });
//     })
// }

// function executeUpdateQueries(res, assetId, updateQueries = [], updateParams = [], updateAssetQuery, updateParamsAsset, connection) {
//     if (!Array.isArray(updateQueries) || !Array.isArray(updateParams)) {
//         return res.status(500).json({ message: 'Invalid parameters for update queries' });
//     }
//     db.plantcare.getConnection((err, connection) => {
...
//         })
//         .catch((err) => {
//             connection.rollback(() => res.status(500).json({ message: 'Error executing ownership update', error: err }));
//         });
//     });
// }

// exports.deleteFixedAsset = (req, res, connection) => {
//     const userId = req.user.id;
//     const { assetId, category } = req.params;
