...
} = require("../validations/currentAsset-validation"); // Import validation schemas
const {
  addFixedAssetSchema,
...
exports.deleteAsset = asyncHandler(async (req, res) => {
  try {
    ...
    // Retrieve the current asset record for the user
    const assets = await currentAssetsDao.getCurrentAsset(
      userId,
      category,
      assetId
    );
    if (assets.length === 0) {
      return res
        .status(404)
        .json({ message: "Asset not found for this user." });
    }
    ...
    if (newNumOfUnit < 0 || newTotal < 0) {
      return res
        .status(400)
        .json({ message: "Invalid operation: insufficient units to deduct." });
    }
    ...
    if (newNumOfUnit === 0 && newTotal === 0) {
      await currentAssetsDao.deleteAsset(userId, category, assetId); // Delete asset
      await currentAssetsDao.insertRecord(
      ...
      // Otherwise, update the asset
      await currentAssetsDao.updateAsset(
        userId,
        category,
        assetId,
        newNumOfUnit,
...
exports.handleAddFixedAsset = asyncHandler(async (req, res) => {
  try {
    ...
    // Convert volume to integer (already validated as a number by Joi)
    const volumeInt = parseInt(volume, 10);

    // Format purchaseDate and expireDate to MySQL datetime format
    ...
    const existingAsset = await currentAssetsDao.checkExistingAsset(
      userId,
      category,
      asset
    );
    ...
    if (existingAsset.length > 0) {
      ...
      const updatedAsset = await currentAssetsDao.updateExistingAsset(
        existingAsset[0],
        numberOfUnits,
        totalPrice,
        volumeInt,
      ...
      return res
        .status(200)
        .json({ status: "success", message: "Asset updated successfully" });
    } else {
      // Asset doesn't exist, insert new asset
      const newAssetId = await currentAssetsDao.addNewAsset(
        userId,
        category,
        asset,
        brand,
      ...
      return res
        .status(200)
        .json({ status: "success", message: "Asset added successfully" });
    }
  } catch (err) {
