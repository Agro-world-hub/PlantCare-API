...
exports.checkEnrollCrop = (cultivationId) => {
    const sql =
        "SELECT cropCalendar, id FROM ongoingcultivationscrops WHERE ongoingCultivationId = ?";
    return query(sql, [cultivationId]);
};

exports.enrollOngoingCultivationCrop = (
    cultivationId,
    cropId,
    extentha,
    extentac,
    ...
    const sql =
        "INSERT INTO ongoingcultivationscrops(ongoingCultivationId, cropCalendar,  extentha, extentac, extentp , startedAt, cultivationIndex) VALUES (?, ?,?,?,?,?,?)";
    return query(sql, [
        cultivationId,
        cropId,
        extentha,
...
exports.updateOngoingCultivationCrop = (
    onCulscropID,
    extentha,
    extentac,
    extentp,
    ...
    return new Promise((resolve, reject) => {
        const sql =
            "UPDATE ongoingcultivationscrops SET extentha = ?, extentac=?, extentp=? WHERE id = ?";
        db.plantcare.query(
            sql,
            [extentha, extentac, extentp, onCulscropID],
            (err, results) => {
                if (err) {
                    reject(err);
                } else {
...
exports.getSlaveCropCalendarDays = (onCulscropID) => {
    return new Promise((resolve, reject) => {
        const sql =
            "SELECT id, days FROM slavecropcalendardays WHERE onCulscropID = ?";
...
exports.updateSlaveCropCalendarDay = (id, formattedDate) => {
    return new Promise((resolve, reject) => {
        const sql =
            "UPDATE slavecropcalendardays SET startingDate = ? WHERE id = ?";
...
exports.getSlaveCropCalendarDaysByUserAndCrop = (
    userId,
    cropCalendarId,
    farmId,
) => {
    return new Promise((resolve, reject) => {
        ...
        db.plantcare.query(
            getOnCulscropIdSql,
            [cropCalendarId, farmId],
            (err, onCulscropResults) => {
                if (err) {
                    reject(err);
                    return;
                }
...
exports.getSlaveCropCalendarPrgress = (userId, cropCalendarId, farmId) => {
    return new Promise((resolve, reject) => {
        ...
        db.plantcare.query(
            sql,
            [userId, cropCalendarId, cropCalendarId, farmId],
            (err, results) => {
                if (err) {
                    reject(err);
                } else {
...
exports.getTaskById = (id) => {
    return new Promise((resolve, reject) => {
        const sql =
            "SELECT taskIndex, status, createdAt, cropCalendarId, days, startingDate, userId FROM slavecropcalendardays WHERE id = ?";
        db.plantcare.query(sql, [id], (err, results) => {
            if (err) {
                reject(err);
            } else {
...
exports.getPreviousTasks = (
    ...
    return new Promise((resolve, reject) => {
        ...
        db.plantcare.query(
            sql,
            [taskIndex, cropCalendarId, userId, onCulscropID],
            (err, results) => {
                if (err) {
                    reject(err);
                } else {
...
exports.updateTaskStatus = (id, status, requestUserId, requestUserOwnerId) => {
    return new Promise((resolve, reject) => {
        ...
        if (requestUserId === requestUserOwnerId) {
            ...
            sql =
                "UPDATE slavecropcalendardays SET status = ?, completedStaffId = ?, createdAt = CURRENT_TIMESTAMP WHERE id = ?";
...
exports.addGeoLocation = (longitude, latitude, onCulscropID) => {
    const sql =
        "UPDATE ongoingcultivationscrops SET longitude = ?, latitude = ? WHERE id = ?";
    return new Promise((resolve, reject) => {
        db.plantcare.query(
            sql,
            [longitude, latitude, onCulscropID],
            (err, results) => {
                if (err) {
                    reject(err);
                } else {
